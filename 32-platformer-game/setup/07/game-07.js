...
Game.Object.prototype = {
    ...
    // Does rectengular collision detection
    collideObject: function (object) {
        if (object.getRight() < this.getLeft() || object.getLeft() > this.getRight() ||
            object.getBottom() < this.getTop() || object.getTop() > this.getBottom()) return false;
        return true;
    },

    // does rectengular collision detection with the center of tje object
    collideObjectCenter: function (object) {
        let center = {
            x: object.getCenterX(),
            y: object.getCenterY()
        };
        if (center.x < this.getLeft() || center.x > this.getRight() ||
            center.y < this.getTop() || center.y > this.getBottom()) return false;
        return true;
    },
...
Game.Grass = function (x, y) {
    Game.Animator.call(this, Game.Grass.prototype.frameSets["wave"], 15);
    this.x = x;
    this.y = y;
};
...
Game.TileSet = function (columns, tileSize) {
    ...
    this.frames = [new f(115,  96, 13, 16, 0, -4),  // idle-left
                   ...
                   new f( 81, 112, 14, 16), new f( 96, 112, 16, 16), // twirl
                   new f(112, 115, 16,  4), new f(112, 124, 16,  4), new f(112, 119, 16,  4), //wave
    ];
};
...
Game.World.prototype = {
    ...
    setup: function (zone) {
        ...
        for (let carrotIndex = zone.carrots.length - 1; carrotIndex > -1; --carrotIndex) {
            let carrot = zone.carrots[carrotIndex];
            this.carrots[carrotIndex] = new Game.Carrot(carrot[0] * this.tileSet.tileSize + 5, carrot[1] * this.tileSet.tileSize - 2);
        }
        ...
        for (let grassIndex = zone.grass.length - 1; grassIndex > -1; --grassIndex) {
            let grass = zone.grass[grassIndex];
            this.grass[grassIndex] = new Game.Grass(grass[0] * this.tileSet.tileSize, grass[1] * this.tileSet.tileSize + 12);
        }
    ...
    update: function () {
        ...
        // test for carrot collision and add carrot count on collision
        for (let carrotIndex = this.carrots.length - 1; carrotIndex > -1; --carrotIndex) {
            let carrot = this.carrots[carrotIndex];
        ...
        for (let grassIndex = this.grass.length - 1; grassIndex > -1; --grassIndex) {
            let grass = this.grass[grassIndex];
            grass.animate();
        }
